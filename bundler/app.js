...
function startService(port) {
      ...
      if (req.method === 'GET') {
        ...
        if (pathname === '/bundle') {
          ...
          webpack(config, (err, stats) => {
            ...
            // step3: uglify
            shelljs.exec('uglifyjs -c -m -o __dist/coord.min.js -- __dist/coord.js');
            // step4: zipping it
            const zip = new JSZip();
            zip.folder('coord-dist').file('entry.js', entryFileContent);
            zip.folder('coord-dist').file('coord.js', readFileSync(join(distPath, './coord.js'), 'utf8'));
            zip.folder('coord-dist').file('coord.js.map', readFileSync(join(distPath, './coord.js.map'), 'utf8'));
            zip.folder('coord-dist').file('coord.min.js', readFileSync(join(distPath, './coord.min.js'), 'utf8'));
            res.writeHead(200, {
  ...
  if (commander.web) {
    ...
    const windowBoundsConfig = require('torchjs/lib/windowBoundsConfig')(
      resolve(app.getPath('userData'), './coord-bundler-config.json')
    );
